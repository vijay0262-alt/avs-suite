// Packaged E2E for all four scan components - with retry for scan init
...
// Wait 120 seconds for full backend initialization (1.7 GB DB)
console.log('Waiting 120s for backend to fully initialize...');
setTimeout(() => {
  ...
  setTimeout(() => {
    ...
    if (responses[1] && responses[1].result) {
      ...
      console.log('   reason=' + (r.reason || '').substring(0, 150));
      console.log('   STATUS: PASS\n');
    } else {
      console.log('1. SECURITY_SCORE: FAIL\n');
    }
    ...
    if (responses[2] && responses[2].result) {
      ...
      console.log('2. DEFENDER_STATUS: FAIL\n');
    }
    ...
    if (responses[5] && responses[5].result) {
      ...
      console.log('5. QUARANTINE_LIST: FAIL\n');
    }
