...
class KnownLocations:
    ...
    @staticmethod
    def get_prefetch_root() -> Path:
        """Get the Windows Prefetch directory path.

        Windows Disk Cleanup cleans this. The .pf files are
        automatically regenerated by Windows as needed.
        """
        return KnownLocations.expand(r"%SystemRoot%\Prefetch")

    # ── Downloaded Program Files ─────────────────────────────────

    @staticmethod
    def get_downloaded_program_files_root() -> Path:
        """Get the Downloaded Program Files directory.

        Legacy ActiveX/IE components. Safe to clean —
        Windows re-downloads as needed.
        """
        return KnownLocations.expand(r"%SystemRoot%\Downloaded Program Files")

    # ── Offline Web Pages ────────────────────────────────────────

    @staticmethod
    def get_offline_web_pages_root() -> Path:
        """Get the Offline Web Pages directory.

        Legacy IE offline content. Safe to clean.
        """
        return KnownLocations.expand(r"%SystemRoot%\Offline Web Pages")

    # ── Font Cache ───────────────────────────────────────────────

    @staticmethod
    def get_font_cache_root() -> Path:
        """Get the Windows Font Cache directory.

        Font cache is auto-regenerated by Windows.
        """
        return KnownLocations.expand(
    ...
    @staticmethod
    def get_branch_cache_root() -> Path:
        """Get the BranchCache directory (if it exists).

        BranchCache is only present if the feature is enabled.
        Safe to clean — content is re-fetched from upstream.
        """
        return KnownLocations.expand(
    ...
    @staticmethod
    def get_retail_demo_root() -> Path:
        """Get the Windows RetailDemo directory.

        Only present on retail demo units. Safe to clean.
        """
        return KnownLocations.expand(
    ...
    @staticmethod
    def get_memory_dump_file() -> Path:
        """Get the system memory dump file path.

        MEMORY.DMP is created on blue screen crashes.
        Safe to clean — it's a diagnostic snapshot.
        """
        return KnownLocations.expand(r"%SystemRoot%\MEMORY.DMP")
    ...
    def get_protected_exceptions() -> list[Path]:
        ...
        candidates = [
            ...
            r"%SystemRoot%\SoftwareDistribution\DeliveryOptimization",
            # V1.0: Windows Disk Cleanup categories under %SystemRoot%
            # that are safe to clean despite the protected parent.
            r"%SystemRoot%\Prefetch",
            r"%SystemRoot%\Downloaded Program Files",
            r"%SystemRoot%\Offline Web Pages",
            r"%SystemRoot%\ServiceProfiles\LocalService\AppData\Local\FontCache",
            r"%SystemRoot%\ServiceProfiles\NetworkService\AppData\Local\BranchCache",
            # V1.0: MEMORY.DMP is a single file under %SystemRoot%.
