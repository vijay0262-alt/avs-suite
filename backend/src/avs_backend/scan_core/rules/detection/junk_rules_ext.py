...
# V1.0: If a browser process is running, its cache files should NOT be
# classified as SAFE for automatic cleaning — even if the individual file
# is not currently locked. The cache could be in use or become locked
# between scan and deletion. Classify as REVIEW_REQUIRED instead.
...
_BROWSER_CACHE_PATH_MARKERS = {
    "google/chrome": "chrome",
    "microsoft/edge": "edge",
    "bravesoftware/brave": "brave",
...
def _detect_running_browsers() -> set[str]:
    ...
    global _cached_running_browsers
    if _cached_running_browsers is not None:
        return _cached_running_browsers
    ...
    try:
        ...
        for proc in psutil.process_iter(attrs=["name"]):
            ...
            for browser_name, exe_names in _BROWSER_PROCESS_NAMES.items():
                if name in exe_names:
                    running.add(browser_name)
                    break
    except Exception:
        pass  # Non-fatal — if we can't check, don't block detection

    _cached_running_browsers = running
    return running


def _browser_for_cache_path(canonical_path: str) -> Optional[str]:
    """Determine which browser a cache path belongs to."""
    path_lower = canonical_path.lower().replace("\\", "/")
    for marker, browser_name in _BROWSER_CACHE_PATH_MARKERS.items():
        if marker in path_lower:
            return browser_name
    return None


def _is_browser_running_for_path(canonical_path: str) -> bool:
    """Check if the browser that owns this cache path is currently running."""
    browser_name = _browser_for_cache_path(canonical_path)
    if browser_name is None:
        return False
    return browser_name in _detect_running_browsers()
...
class BrowserCacheRule(Rule):
    ...
    def evaluate(
        ...
        # V1.0: If the owning browser is currently running, override safety
        # to REVIEW_REQUIRED. Browser cache files (especially LevelDB/
        # CacheStorage) can become locked between scan and deletion even if
        # not locked at scan time. Do NOT automatically clean active browser
        # cache — require the user to close the browser first.
        if safety.is_safe and _is_browser_running_for_path(asset.canonical_path):
            from ..safety import SafetyAssessment
            from ..enums import SafetyBlocker
            safety = SafetyAssessment.create_review_required(
                reason=(
                    "Browser is currently running — close the browser "
                    "before cleaning its cache to avoid deletion failures"
                ),
            )
            evidence_items.append(
                Evidence(
                    evidence_type=EvidenceType.METADATA_MATCH,
                    source=self.rule_id,
                    description="Owning browser process is running — cache may be in use",
