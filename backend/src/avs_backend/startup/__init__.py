...
def _update_cached_entry(entry_data: dict[str, Any], enabled: bool) -> None:
    """Flip a cached entry's enabled flag in place so the next
    ``startup.list`` is instant — no full rescan after every toggle."""
    with _cache_lock:
        if _startup_cache is None:
            return
        for e in _startup_cache:
            if e["name"] == entry_data.get("name") and e["location"] == entry_data.get("location"):
                e["enabled"] = enabled
                e["status"] = "enabled" if enabled else "disabled"
...
def startup_disable(params: dict[str, Any] | None) -> dict[str, Any]:
    ...
    try:
        ...
        # Update the cached entry in place — the toggle is already
        # reflected on disk (StartupApproved flag / moved shortcut),
        # so a full rescan would just add latency.
        if isinstance(result, dict) and result.get("success", False):
            _update_cached_entry(entry_data, enabled=False)
...
def startup_enable(params: dict[str, Any] | None) -> dict[str, Any]:
    ...
    try:
        ...
        # Update the cached entry in place — no rescan needed.
        if success:
            _update_cached_entry(entry_data, enabled=True)
...
def startup_restore(params: dict[str, Any] | None) -> dict[str, Any]:
    ...
    try:
        ...
        # Restore changes on-disk state — invalidate so the next list is fresh.
        global _startup_cache, _cache_timestamp
        with _cache_lock:
            _startup_cache = None
            _cache_timestamp = 0
