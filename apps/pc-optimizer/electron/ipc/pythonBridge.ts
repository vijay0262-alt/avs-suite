  ...
  const client: RpcClient = {
    call<T>(method: string, params?: unknown, customTimeoutMs?: number): Promise<T> {
      ...
      const doCall = (attempt: number): Promise<T> => new Promise<T>((resolve, reject) => {
        ...
        const isLongOperation = method.includes('optimize') || method.includes('clean') || method.includes('execute') || method.includes('analyze') || method.includes('scan') || method.includes('dashboard.') || method.includes('metrics') || method.includes('orchestrator.') || method.includes('one_click.');
