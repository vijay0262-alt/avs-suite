  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-text-primary">AI Verdict</h3>
        <div className="ml-auto flex items-center gap-1.5">
          <span className="text-xs text-text-muted">Confidence</span>
          <span className="text-sm font-bold tabular-nums text-brand-primary" aria-live="polite">
            {displayConfidence}%
          </span>
        </div>
      </div>
      ...
      {verdict.evidenceSources.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 pt-1 border-t border-brand-primary/10">
          ...
          {verdict.evidenceSources.map((source, i) => (
            <span
              key={i}
              className="rounded-full bg-[var(--avs-surface-muted)] px-2 py-0.5 text-[10px] font-medium text-text-secondary"
            >
              {source}
            </span>
          ))}
          <span className="text-[10px] text-text-muted">· {verdict.evidenceCount} data points</span>
        </div>
      )}
    </div>
  );
