    ...
    <div data-testid="overview-section" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <Card data-testid="overview-health">
        <div className="flex items-center justify-between mb-2">
          <span className="text-caption font-medium uppercase tracking-wide text-text-muted">Health Score</span>
          <Badge tone={healthTone(overview.healthLevel)}>{overview.healthLevel}</Badge>
        </div>
        <div className="text-statistic text-text-primary">{overview.healthScore}</div>
        <div className="mt-2">
          <ProgressBar
      ...
      <Card data-testid="overview-temperature">
        <div className="flex items-center justify-between mb-2">
          <span className="text-caption font-medium uppercase tracking-wide text-text-muted">System Temperature</span>
          <Badge tone={healthTone(overview.overallTempLevel)}>{overview.overallTempLevel}</Badge>
        </div>
        <div className="text-statistic text-text-primary">
      ...
      <Card data-testid="overview-power-cooling">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-caption font-medium uppercase tracking-wide text-text-muted">Power Status</span>
            <Badge tone={statusTone(overview.powerStatus)}>{overview.powerStatus}</Badge>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-caption font-medium uppercase tracking-wide text-text-muted">Cooling Status</span>
            <Badge tone={statusTone(overview.coolingStatus)}>{overview.coolingStatus}</Badge>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-caption font-medium uppercase tracking-wide text-text-muted">System Uptime</span>
      ...
      <Card data-testid="overview-providers">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-caption font-medium uppercase tracking-wide text-text-muted">Last Scan</span>
            <span className="text-small text-text-primary">
              {new Date(overview.lastScanAt).toLocaleTimeString()}
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-caption font-medium uppercase tracking-wide text-text-muted">Sensors</span>
            <span className="text-small font-medium text-text-primary">
          ...
          {overview.providerStatuses.length > 0 && (
            <div>
              <div className="text-caption font-medium uppercase tracking-wide text-text-muted mb-1">Providers</div>
